...
def hist_price_dl(coin_id=1, timeframe="5y", currency="USD"):
    ...
    try:
        with open('coin_history.json', 'r') as infile:
            coin = json.load(infile)
            print("loaded coin_history.json from disk")
        ...
        with open("coin_history.json", 'w') as outfile:
            json.dump(coin, outfile)
            print("saved coin_history.json to disk")
